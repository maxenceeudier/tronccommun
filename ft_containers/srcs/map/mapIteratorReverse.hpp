...
namespace ft
{
    ...
    public:
        ...
        mapIteratorReverse(node_pointer ptr) : _is_gost(false), _alloc(Allocator()) {node = ptr;};
        mapIteratorReverse(node_const_pointer ptr) : _is_gost(false), _alloc(Allocator()) {node = ptr;};
        mapIteratorReverse(mapIteratorReverse const &src): _is_gost(src._is_gost), _alloc(Allocator())
        {
            if (this->_is_gost)
            {
